...
export async function saveProgress(fileId: string, scrollTop: number, scrollHeight: number): Promise<void> {
  const db = await getDB();
  await db.put('progress', { fileId, scrollTop, scrollHeight, updatedAt: Date.now() });
}
